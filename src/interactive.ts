 ...
 */
function validatePrInput(input: string): {
  valid: boolean;
  repo?: string;
  isUrl: boolean;
} {
...
export async function runInteractive(): Promise<InteractiveResult> {
  ...
  try {
    ...
    while (!prUrlOrNumber) {
      ...
      if (!validation.valid) {
        console.log(
          "❌ Invalid input. Please enter a GitHub PR URL or a PR number.\n"
        );
        continue;
      }
    ...
    const autoGuidelines = await getGuidelinesForRepo(
      detectedRepo || prUrlOrNumber,
      config
    );
    ...
    if (autoGuidelines) {
      ...
      console.log(
        `  1. Auto-detected for this repo: ${autoGuidelines} ${
          exists ? "✓" : "(not found)"
        }`
      );
    }
    if (config.defaultGuidelines) {
      ...
      console.log(
        `  2. Default guidelines: ${config.defaultGuidelines} ${
          exists ? "✓" : "(not found)"
        }`
      );
    }
    ...
    if (detectedRepo && contextPath) {
      const save = await question(
        rl,
        `Save these settings for ${detectedRepo}? (y/n): `
      );
